        ...
        if keyframe_points.shape == normalized_keypoints.shape:
            ...
            if distance < 0.9:
                ...
                if state["frame_stage"] == 3:
                    if(len(state["detected_words"])==0 or state["detected_words"][-1]!=state["pending_gesture"]):
                        state["detected_words"].append(state["pending_gesture"])
